...
// ✅ ADD: Candidate model (DB fetch)
const Candidate = require("../models/Candidate"); // <-- if your model file name differs, update this path
...
/**
 * ✅ NEW helper: load candidate from DB and normalize fields.
 * Adjust the field names here ONCE if your Candidate schema differs.
 */
async function loadCandidateFromDb(candidateId) {
  ...
  const phone = candidate.phone || candidate.mobile || "";
  ...
  const summary = candidate.summary || candidate.professionalSummary || "";

  const certifications =
    (Array.isArray(candidate.certifications) && candidate.certifications) ||
    (typeof candidate.certifications === "string"
      ? candidate.certifications.split(",").map((s) => s.trim()).filter(Boolean)
      : []);
...
exports.generateResume = async (req, res) => {
  try {
    ...
    // ✅ UPDATED: fetch candidate from DB
    const { candidateId, targetRole, jobDescription } = req.body;
    ...
    const db = await loadCandidateFromDb(candidateId);
    if (!db) {
      return res.status(404).json({ message: "Candidate not found" });
    }

    // Allow req.body overrides, otherwise use DB
    const fullName = req.body.fullName || db.fullName;
    const email = req.body.email || db.email;
    const phone = req.body.phone || db.phone;
    const location = req.body.location || db.location;

    const experience = Array.isArray(req.body.experience) && req.body.experience.length > 0
      ? req.body.experience
      : db.experience;

    const education = Array.isArray(req.body.education) && req.body.education.length > 0
      ? req.body.education
      : db.education;
    ...
    const summary = req.body.summary || db.summary;

    const certifications = (Array.isArray(req.body.certifications) && req.body.certifications.length > 0)
      ? req.body.certifications
      : db.certifications;

    if (!fullName) {
      return res.status(400).json({ message: "Candidate name is missing in DB" });
    }

    if (!Array.isArray(experience) || experience.length === 0) {
      return res.status(400).json({
        message:
          "Candidate experience is missing in DB (company/title/dates must be saved for the candidate).",
      });
    }
    ...
    if (!Array.isArray(education) || education.length === 0) {
      return res.status(400).json({
        message:
          "Candidate education is missing in DB (school/degree/year must be saved for the candidate).",
      });
    }
